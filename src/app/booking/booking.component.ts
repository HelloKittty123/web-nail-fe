...
export class BookingComponent implements OnInit {
  ...
  ExpandCategory(category: any) {
    ...
    this.categories
      ...
      .forEach((element) => (element.isExpand = false));
  }

  ProcessQuantity() {
    this.totalService = 0;
    this.categories.forEach((category) => {
      category.totalAmount = (category.items as any[]).filter(
        (x) => x.isSelected
      ).length;
    });
  }
  ...
  GetGallery() {
    this._service.GetGallery().subscribe((response: any) => {
      if (response.panel != null)
        response.panel.url = `${API_END_POINT}${response.panel.url}`;

      if (response.first != null)
        response.first.url = `${API_END_POINT}${response.first.url}`;

      if (response.second != null)
        response.second.url = `${API_END_POINT}${response.second.url}`;

      if (response.third != null)
        response.third.url = `${API_END_POINT}${response.third.url}`;

      if (response.fourth != null)
        response.fourth.url = `${API_END_POINT}${response.fourth.url}`;

      if (response.fifth != null)
        response.fifth.url = `${API_END_POINT}${response.fifth.url}`;

      if (response.sixth != null)
        response.sixth.url = `${API_END_POINT}${response.sixth.url}`;

      if (response.seventh != null)
        response.seventh.url = `${API_END_POINT}${response.seventh.url}`;

      if (response.eighth != null)
        response.eighth.url = `${API_END_POINT}${response.eighth.url}`;
  ...
  Save() {
    ...
    this.calendar.staffName = this.staffs.find(
      (x) => x.id == this.calendar.staffId
    ).name;
    ...
    this._service.CreateCalendar(request).subscribe((response: any) => {
      if (response.isSuccess) {
        ...
        this._toast.error(
          'The employee already has a work schedule during the time period you selected',
          'We are sorry'
        );
      }
    });
  ...
  OnChangeViewDate(date: any) {
    ...
    this._service.GetCalendar(request).subscribe((response: any) => {
      ...
      this.events = [];
      data
        .filter((x) => x.status == 1 || x.status == 3)
        .forEach((element) => {
  ...
  GetCalendar(event?: any) {
    ...
    let request = {
      ...
      end: moment(this.calendar.start)
        .endOf('week')
